...
// TX Power tables
// The RF_TxPowerTable_DEFAULT_PA_ENTRY and RF_TxPowerTable_HIGH_PA_ENTRY macros are defined in
// RF.h. The following arguments are required: RF_TxPowerTable_DEFAULT_PA_ENTRY(bias, gain, boost,
// coefficient) RF_TxPowerTable_HIGH_PA_ENTRY(bias, ibboost, boost, coefficient, ldoTrim) See the
// Technical Reference Manual for further details about the "txPower" Command field. The PA settings
// require the CCFG_FORCE_VDDR_HH = 0 unless stated otherwise.
...
RF_Mode Prop0_mode433 = {
    .rfMode = RF_MODE_AUTO, .cpePatchFxn = &rf_patch_cpe_prop, .mcePatchFxn = 0, .rfePatchFxn = 0};

// Overrides for CMD_PROP_RADIO_DIV_SETUP_PA
uint32_t pOverrides433Sys[] = {
    // override_tc112.json
    // Tx: Configure PA ramp time, PACTL2.RC=0x3 (in ADI0, set PACTL2[4:3]=0x3)
    ADI_2HALFREG_OVERRIDE(0, 16, 0x8, 0x8, 17, 0x1, 0x1),
    // Rx: Set AGC reference level to 0x1C (default: 0x2E)
    HW_REG_OVERRIDE(0x609C, 0x001C),
    // Rx: Set RSSI offset to adjust reported RSSI by -6 dB (default: 0), trimmed for external bias
    // and differential configuration
    (uint32_t)0x000688A3,
    ...
    // DC/DC regulator: In Tx with 14 dBm PA setting, use DCDCCTL5[3:0]=0xF (DITHER_EN=1 and
    // IPEAK=7). In Rx, use default settings.
    (uint32_t)0x00F788D3,
    // override_prop_common_sub1g.json
    // TX: Set FSCA divider bias to 1
    HW32_ARRAY_OVERRIDE(0x405C, 0x0001),
    // TX: Set FSCA divider bias to 1
    (uint32_t)0x08141131, (uint32_t)0xFFFFFFFF};

// CMD_PROP_RADIO_DIV_SETUP_PA
// Proprietary Mode Radio Setup Command for All Frequency Bands
rfc_CMD_PROP_RADIO_DIV_SETUP_PA_t Prop0_cmdPropRadioDivSetup433 = {.commandNo = 0x3807,
                                                                   .status = 0x0000,
                                                                   .pNextOp = 0,
...
rfc_CMD_FS_t Prop0_cmdFs433 = {.commandNo = 0x0803,
                               .status = 0x0000,
                               .pNextOp = 0,
                               .startTime = 0x00000000,
                               .startTrigger.triggerType = 0x0,
...
rfc_CMD_PROP_TX_t Prop0_cmdPropTx433 = {.commandNo = 0x3801,
                                        .status = 0x0000,
                                        .pNextOp = 0,
                                        .startTime = 0x00000000,
                                        .startTrigger.triggerType = 0x0,
...
rfc_CMD_PROP_RX_t Prop0_cmdPropRx433 = {.commandNo = 0x3802,
                                        .status = 0x0000,
                                        .pNextOp = 0,
                                        .startTime = 0x00000000,
                                        .startTrigger.triggerType = 0x0,
