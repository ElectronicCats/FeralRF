...
class Radio:
    ...
    def __init__(self, port: Optional[str] = None, baudrate: int = 921600):
        ...
        # Buffer for async events (seq=0 frames, e.g. RSP_ERROR emitted
        # outside a command/response cycle) that arrive while _read_response
        # is waiting for something else. Without this, async events are
        # silently dropped because their seq doesn't match _last_seq and
        # their cmd_id doesn't match `expected`. A later _read_response call
        # with matching `expected` drains this buffer first, then reads from
        # serial. Keyed by cmd_id; each value is a deque of (cmd_id, seq,
        # payload) tuples in arrival order.
    ...
    def _read_response(
        ...
        # different opcode while an async event (seq=0) arrived. Without
        # this drain, async events would be lost between the call that saw
        # them and the caller that wants them.
        if expected:
            ...
            if byte == b"\x00" and len(self._rx_buffer) > 1:
                ...
                try:
                    ...
                    if expected is not None and cmd_id not in expected:
                        ...
                        # async frames that race with synchronous
